...
;;I really like working with lots of windows, I also really like this plugin so I hated how it deleted all my windows.  After looking into the code I see why you did it like you did.

;;I created an optional mode that saves your window configuration, opens nav, lets you open
...
(defcustom nav-remember-windows t
  "*If non-nil, nav will attempt to remember your window configuration, and restore it when you exit."
  :type 'boolean
  :group 'nav)
...
(defun nav-open-file (filename)
  ...
  (if (file-directory-p filename)
    ...
    (setq nav-buffer-recent-file 'filename)
    (message 'filename)
...
(defun nav-open-file-under-cursor ()
  ...
  (let ((filename (nav-get-cur-line-str)))
    (setq nav-buffer-recent-file '(concat dirname "/" filename))
    (message "Saveing recent file")
...
(defun nav-open-file-other-window (k)
  (let ((filename (nav-get-cur-line-str))
    ...
    (setq nav-buffer-recent-file '(concat dirname "/" filename))
    (message "Saveing recent file")
    ;;(custom-set-variables '(nav-buffer-recent-file (concat dirname "/" filename)))
    (other-window k)
...
(defun nav-quit ()
  ...
  (when nav-remember-windows
    (jump-to-register ?u)
  (if (string= nav-buffer-recent-file "1")
    (message "No Opened File.")
    (ding)    
    (find-file nav-buffer-recent-file))))
